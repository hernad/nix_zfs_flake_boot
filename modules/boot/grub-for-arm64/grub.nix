...
# https://github.com/NixOS/nixpkgs/blob/release-22.11/nixos/modules/system/boot/loader/grub/grub.nix
{ config, lib, pkgs, ... }:
  ...
  grubConfig = args:
    ...
    in pkgs.writeText "grub-config.xml" (builtins.toXML {
      ...
      grubTargetEfi = if cfg.efiSupport && (cfg.version == 2) then
        ...
        "";
      bootPath = args.path;
      storePath = config.entry.aarch64Grub.storePath;
...
in {
  ...
  config = mkMerge [
    ...
    {
      entry.aarch64Grub.splashImage = mkDefault (if cfg.version == 1 then
        pkgs.fetchurl {
          url =
    ...
    (mkIf (cfg.splashImage == defaultSplash) {
      entry.aarch64Grub.backgroundColor = mkDefault "#2F302F";
      entry.aarch64Grub.splashMode = mkDefault "normal";
    })

    (mkIf cfg.enable {

      entry.aarch64Grub.devices = optional (cfg.device != "") cfg.device;

      entry.aarch64Grub.mirroredBoots = optionals (cfg.devices != [ ]) [{
        path = "/boot";
        inherit (cfg) devices;
      ...
      entry.aarch64Grub.extraPrepareConfig = concatStrings (mapAttrsToList
        (n: v: ''
          ${pkgs.coreutils}/bin/cp -pf "${v}" "@bootPath@/${n}"
        '') config.entry.aarch64Grub.extraFiles);
      ...
      assertions = [
        ...
        }
        {
          ...
          message = "You must set the option ‘entry.aarch64Grub.devices’ or "
            + "'entry.aarch64Grub.mirroredBoots' to make the system bootable.";
        }
        {
        ...
        }
        {
          ...
          message =
            "If you wish to to use entry.aarch64Grub.efiInstallAsRemovable, then turn on entry.aarch64Grub.efiSupport";
        }
        {
          assertion = cfg.efiInstallAsRemovable
            -> !config.boot.loader.efi.canTouchEfiVariables;
          message =
            "If you wish to to use entry.aarch64Grub.efiInstallAsRemovable, then turn off boot.loader.efi.canTouchEfiVariables";
        }
